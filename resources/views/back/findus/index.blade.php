...
@section('main')
  ...
  @include('back.partials.entete', ['title' => trans('back/all.dashboard') . link_to_route('adm_findus.create', trans('back/all.add'), [], ['class' => 'btn btn-info pull-right']), 'icon' => 'pencil', 'fil' => trans('back/all.table')])
        ...
        <thead>
          <tr>
              ...
              <th>
                  title_fr
                  <a href="#" name="title" class="order">
                      <span class="fa fa-fw fa-{{ $order->name == 'finduss.fr_title' ? $order->sens : 'unsorted'}}"></span>
                  </a>
              </th>
              <th>
                  title_de
                  <a href="#" name="title" class="order">
                      <span class="fa fa-fw fa-{{ $order->name == 'finduss.de_title' ? $order->sens : 'unsorted'}}"></span>
                  </a>
              </th>
              <th>
                  Sort
                  <a href="#" name="sort" class="order">
                      <span class="fa fa-fw fa-{{ $order->name == 'finduss.sort' ? $order->sens : 'unsorted'}}"></span>
                  </a>
              </th>
            <th>
              {{ trans('back/all.date') }}
              <a href="#" name="created_at" class="order">
                <span class="fa fa-fw fa-{{ $order->name == 'finduss.created_at' ? $order->sens : 'unsorted'}}"></span>
              </a>
            </th>
            <th>
              {{ trans('back/all.published') }}
              <a href="#" name="collections.active" class="order">
                <span class="fa fa-fw fa-{{ $order->name == 'finduss.active' ? $order->sens : 'unsorted'}}"></span>
              </a>
            </th> 
            @if(session('statut') == 'admin')
              <th>
                ...
                <a href="#" name="seen" class="order">
                  <span class="fa fa-fw fa-{{ $order->name == 'finduss.seen' ? $order->sens : 'unsorted'}}"></span>
                </a>
              </th>
            @endif
              <th>
                  is_menu
                  <a href="#" name="finduss.is_menu" class="order">
                      <span class="fa fa-fw fa-{{ $order->name == 'finduss.is_menu' ? $order->sens : 'unsorted'}}"></span>
                  </a>
              </th>
            <th></th>
            <th></th>
  ...
  <script>
    ...
    $(function() {
        ...
        $(document).on('change', ':checkbox[name="seen"]', function() {
            ...
            $.ajax({
                url: '{{ url('adm_finduspostseen') }}' + '/' + this.value,
                type: 'PUT',
        ...
        $(document).on('change', ':checkbox[name="active"]', function() {
            $(this).hide().parent().append('<i class="fa fa-refresh fa-spin"></i>');
            $.ajax({
                url: '{{ url('adm_finduspostactive') }}' + '/' + this.value,
                type: 'PUT',
        ...
        $(document).on('change', ':checkbox[name="is_menu"]', function() {
            ...
            $(this).hide().parent().append('<i class="fa fa-refresh fa-spin"></i>');
            $.ajax({
                url: '{{ url('adm_finduspostis_menu') }}' + '/' + this.value,
                type: 'PUT',
        ...
        $('a.order').click(function(e) {
            ...
            // Send ajax      
            $.get('{{ url('adm_findus/order') }}', { name: name, sens: tri })
            .done(function(data) {

            ...
            <div class="col-lg-12">
                ...
                @if($post->tags->count())
                    <div class="text-center">
                        <small>{{ trans('front/testblock.tags') }}</small>
                        @foreach($post->tags as $tag)
                            {!! link_to('testblock/tag?tag=' . $tag->id, $tag->tag, ['class' => 'btn btn-default btn-xs']) !!}
                        @endforeach
                    </div>
                @endif
            </div>
    ...
    <div class="row">
        <div class="box">
            <div class="col-lg-12">
                <div class="col-lg-12">
                    <hr>
                    <h3 class="text-center">{{ trans('front/testblock.comments') }}</h3>
                    <hr>
                    ...
                    @if($comments->count())
                        @foreach($comments as $comment)
                            <div class="commentitem">
                                <h3>
                                    <small>{{ $comment->user->username . ' ' . trans('front/testblock.on') . ' ' . $comment->created_at }}</small>
                                    @if($user && $user->username == $comment->user->username) 
                                        <a id="deletecomment{!! $comment->id !!}" href="#" class="deletecomment"><span class="fa fa-fw fa-trash pull-right" data-toggle="tooltip" data-placement="left" title="{{ trans('front/testblock.delete') }}"></span></a>
                                        <a id="comment{!! $comment->id !!}" href="#" class="editcomment"><span class="fa fa-fw fa-pencil pull-right" data-toggle="tooltip" data-placement="left" title="{{ trans('front/testblock.edit') }}"></span></a>
                                    @endif
                                </h3>
                    ...
                    <div class="row" id="formcreate"> 
                        ...
                        @if(session('statut') != 'visitor')
                            {!! Form::open(['url' => 'comment']) !!}    
                                ...
                                {!! Form::controlBootstrap('textarea', 12, 'comments', $errors, trans('front/testblock.comment')) !!}
                                {!! Form::submitBootstrap(trans('front/form.send'), 'col-lg-12') !!}
                            {!! Form::close() !!}
                        @else
                            <div class="text-center"><i class="text-center">{{ trans('front/testblock.info-comment') }}</i></div>
                        @endif
                    </div>
    ...
    <script>      
        ...
        @if(session('statut') != 'visitor')
            ...
            $(function() {
                ...
                function buttons(i) {
                    return "<input id='val" + i +"' class='btn btn-default' type='submit' value='{{ trans('front/testblock.valid') }}'><input id='btn" + i + "' class='btn btn-default btnannuler' type='button' value='{{ trans('front/testblock.undo') }}'></div>";
                }
                ...
                $('a.deletecomment').click(function(e) {   
                    ...
                    var that = $(this);
                    swal({
                        title: "{!! trans('front/testblock.confirm') !!}",
                        type: "warning",
                        ...
                        if (isConfirm) {
                            ...
                            .fail(function() {
                                alert('{!! trans('front/testblock.fail-delete') !!}');
                            }); 
                        }                
                    });
                });

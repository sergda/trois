...
class FindUsRepository extends BaseRepository
{
    protected $imagesProject;
    
    public function __construct(Findus $post, ImagesProject $imagesProject)
    {
        $this->model = $post;
    ...
    {
        $query = $this->model
            ->select('finduss.id', 'finduss.created_at', 'en_title', 'en_description', 'en_keywords', 'finduss.seen', 'active', 'is_menu', 'user_id', 'slug', 'username')
